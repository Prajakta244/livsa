    ...
    return (
        ...
        <tbody>
            {daysArr.map((k, i) => (
                <tr key={i} >
                    <>
                        <td className='py-2 border-b-[1px] border-gray-300 dark:border-gray-700'> <h3 className='text-[14px] mx-4'>{k}</h3></td>
                        <td className='border-b-[1px] border-gray-300 dark:border-gray-700'>
                            <CustomFormField
                                ...
                                onChange={(date) => {
                                    if (k == 'Mon - Fri') {
                                        setWeekDayObj({...weekDayObj,...weekDayArr.reduce((curr, m) => {
                                        ...
                                        setWeekDayObj({...weekDayObj,...weekendArr.reduce((curr, m) => {
                                            curr[m] = `${date.toLocaleTimeString()} - ${weekDayObj[m].split(' - ')[1]}`
                                            return curr
                                        }, {}
                                        )})
